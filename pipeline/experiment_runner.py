...
class ExperimentRunner:
    """Orchestrates: per-experiment windowing → scaling → fit → predict → metrics.

    Windowing lives here (not in a preprocessor) because it changes sample count
    and must happen with per-experiment input_len/horizon overrides. This keeps
    the BasePreprocessor contract shape-preserving as specified.

    Validation data, if provided, is windowed and scaled with the same fitted
    preprocessor, then forwarded to model.fit() via kwargs. Models that need it
    (NNs with early stopping) consume X_val/y_val; baselines ignore them via **kwargs.
    """
    ...
    def run(self, experiment, X_train, y_train, X_test, y_test,
        ...
        experiment.preprocessor.fit(Xw_tr)
